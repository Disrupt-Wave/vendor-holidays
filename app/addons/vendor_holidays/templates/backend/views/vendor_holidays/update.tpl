...
<div id="content_general">
    <fieldset>
        <div class="control-group">
            <label class="control-label" for="elm_holiday_vendor_id">{__("vendor")}:</label>
            <div class="controls">
                {if $auth.user_type == "A"}
                    {include file="views/companies/components/picker/picker.tpl"
                        type="single"
                        item_ids=[$holiday.vendor_id]
                        view_mode="single_button"
                        show_advanced=false
                        input_name="holiday_data[vendor_id]"
                        item_ids=[$holiday.vendor_id]
                    }
                {else}
                    <input type="hidden" name="holiday_data[vendor_id]" value="{$auth.company_id}" />
                    {$auth.company_id}
                {/if}
            </div>
        </div>
